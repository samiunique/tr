...
char **get_environ(in_formations *info)
{
...
int _setenv(in_formations *info, char *var, char *value)
{
	char *buf = NULL;
	list_stru *node;
	char *p;
	...
	buf = malloc(my_string_length(var) + my_string_length(value) + 2);
	if (!buf)
		return (1);
 ...
 */
int _unsetenv(in_formations *info, char *var)
{
