...
/**
 * string_split - splits a string into words specified by a delimiter(s),
...
char **string_split(char *str, char *separators, size_t *word_count)
{
	...
	char **words;
	char *str_ptr = str;
	unsigned int c, word_sizes[MAX_WORD_C];

	set_zero(word_sizes, MAX_WORD_C);
	...
	for (v = 0; v < no_of_words; v++)
	{
		...
		{
			while (is_delimit(*str_ptr, separators))
				str_ptr++;
...
#include "shell.h"
/**
 * **into_two - splits a string into words. Repeat delimiters are ignored
 * @str: the input string
 * @d: the delimeter string
 * Return: a pointer to an array of strings, or NULL on failure
 */
char **into_two(char *str, char *d)
{
